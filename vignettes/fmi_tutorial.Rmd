...
Note that the actual location of GDAL may vary depending on your system. To test that the tools are found,
type the command in terminal (Command Prompt program in Windows):
```
ogr2ogr
```
...
ogr2ogr --help
```
and you should see options `-splitlistfields` and `-explodecollections` in the help.
...
head(as.data.frame(response)[,c("coords.x1","coords.x2","name1","time","result_MeasurementTimeseries_point_MeasurementTVP_value")])
```
...
head(as.data.frame(response)[,c("coords.x1","coords.x2","name1","time1","variable","measurement1","measurement2")])
```
